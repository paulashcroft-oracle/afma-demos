...
Resolve a distinct AIDEMODB source-app service client with `feedback.write` only. The current forwarder sends an intake POST and validates its immediate response; it has no subsequent feedback read, response poller or response-ACK consumer. The three read/response scopes in the earlier test plan are not prerequisites for this migration. Add them only for a separately established consumer requirement. The AI Hub coordinator owns discovery/provisioning of the credential through an existing authorized administration route. Never install the `codex-caab` automation key or a legacy ASHCROFT key in the app.

The coordinator's 17 September source-only proposal names client `caab-apex-feedback` and a new Web Credential static ID `AI_HUB_CAAB_FEEDBACK_AIDEMODB`. Before creation, verify that the new static ID is absent in AFMA. Proposed header: `X-AI-Hub-API-Key`; proposed valid-URL prefix: the exact canonical feedback endpoint below. These are planned values, not issued-key, installation or activation evidence. Preserve legacy `AI_HUB_AFMA_FEEDBACK_API` unchanged. Original R09 authorization covers ordinary issuance of this appropriate credential through an existing authorized administration route. The actual executor and supported secret-preserving installation mechanism remain to be resolved through the coordinator; never transfer the raw value through chat, SQL text or browser automation payloads. A direct-SQL security-table fallback or broader grants retain their separate named decision.
...
Use one positive/praise intake for the controlled proof. Here, acknowledgement means the immediate intake result: accepted status, project `caab`, the exact idempotency key, a valid task or feedback identifier, and a public response for response-only feedback. Verify the stored local result and identical identity on forced replay. Confirm duplicate counts through an already authorized AI Hub inspection path; do not broaden the source-app key solely to perform the test.

The earlier plan added a separate structured-response GET/ACK round trip without a CAAB consumer requirement. That is not an R09 acceptance gate and no polling, response display or response-authoring feature is introduced here. Preserve the live API boundary and reconcile uncertain intake results before another business request.
