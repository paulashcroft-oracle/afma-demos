...
create or replace package body csiro_caab_page_api as
  ...
  begin
    ...
    append_line(p_html, '.afma-caab-message--user{margin-left:auto;background:#e8f7fa;border-color:#bfe7ee}.afma-caab-label{font-size:.75rem;font-weight:800;text-transform:uppercase;color:var(--afma-muted);margin-bottom:.4rem}');
    ...
    append_line(p_html, '.afma-caab-markdown h1,.afma-caab-markdown h2,.afma-caab-markdown h3{color:var(--afma-navy)}.afma-caab-markdown table{border-collapse:collapse;width:100%;margin:.75rem 0}.afma-caab-markdown th,.afma-caab-markdown td{border:1px solid var(--afma-line);padding:.45rem}.afma-caab-markdown img{max-width:min(100%,42rem);border-radius:8px;border:1px solid var(--afma-line)}');
  ...
  begin
    ...
    append_line(l_html, 'function esc(v){return String(v||"").replace(/[&<>"'']/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","''":"&#39;"}[c];});}');
    append_line(l_html, 'function safeHref(v){var h=String(v||"").trim();return /^https?:\/\//i.test(h)?h:"";}');
    append_line(l_html, 'function md(v){var s=esc(v||"");s=s.replace(/```([\s\S]*?)```/g,function(_,c){return "<pre><code>"+c+"</code></pre>";});s=s.replace(/^### (.*)$/gm,"<h3>$1</h3>").replace(/^## (.*)$/gm,"<h2>$1</h2>").replace(/^# (.*)$/gm,"<h1>$1</h1>");s=s.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/`([^`]+)`/g,"<code>$1</code>");s=s.replace(/!\[([^\]]*)\]\((https?:\/\/[^)\s]+)\)/g,function(_,a,u){return "<img src=\""+safeHref(u)+"\" alt=\""+esc(a)+"\" loading=\"lazy\">";});s=s.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g,function(_,a,u){return "<a href=\""+safeHref(u)+"\" target=\"_blank\" rel=\"noopener\">"+a+"</a>";});s=s.split(/\n{2,}/).map(function(p){if(/^<h|^<pre|^<img/.test(p)){return p;} if(/^\s*[-*] /m.test(p)){return "<ul>"+p.split(/\n/).filter(Boolean).map(function(x){return "<li>"+x.replace(/^\s*[-*] /,"")+"</li>";}).join("")+"</ul>";} return "<p>"+p.replace(/\n/g,"<br>")+"</p>";}).join("");return s;}');
