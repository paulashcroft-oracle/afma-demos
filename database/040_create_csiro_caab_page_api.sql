...
create or replace package body csiro_caab_page_api as
  ...
  begin
    ...
    append_line(p_html, '.afma-caab-agent{display:grid;grid-template-rows:auto auto 1fr auto;gap:1rem;min-height:calc(100vh - 12rem)}');
    append_line(p_html, '.afma-caab-toolbar{display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;border-bottom:1px solid var(--afma-line);padding-bottom:.8rem}.afma-caab-toolbar h1{margin:0;color:var(--afma-navy);font-size:clamp(1.6rem,3vw,2.4rem)}');
    ...
    append_line(p_html, '.afma-caab-model{display:flex;gap:.6rem;align-items:center;flex-wrap:wrap;border:1px solid var(--afma-line);border-radius:8px;background:#fff;padding:.65rem .75rem}.afma-caab-model label{font-size:.78rem;font-weight:850;text-transform:uppercase;color:var(--afma-muted)}.afma-caab-model select{min-width:min(100%,20rem);border:1px solid var(--afma-line);border-radius:8px;padding:.45rem .55rem;background:#fff;color:var(--afma-navy)}');
    append_line(p_html, '.afma-caab-thread{border:1px solid var(--afma-line);border-radius:8px;background:var(--afma-soft);padding:1rem;overflow:auto;min-height:28rem}.afma-caab-message{max-width:min(78rem,100%);border:1px solid var(--afma-line);background:#fff;border-radius:8px;padding:1rem;margin-bottom:.85rem;box-shadow:0 8px 22px rgba(24,58,84,.06)}');
    ...
    append_line(p_html, '.afma-caab-composer{display:grid;grid-template-columns:1fr auto;gap:.75rem;align-items:end}.afma-caab-input{min-height:4rem;resize:vertical;border:1px solid var(--afma-line);border-radius:8px;padding:.8rem;font:inherit}.afma-caab-send{height:3rem;border:0;border-radius:8px;background:var(--afma-teal);color:white;font-weight:850;padding:0 1.1rem}');
  ...
  begin
    ...
    append_line(l_html, q'~function md(v){var lines=String(v||"").replace(/\r\n?/g,"\n").split("\n"),out=[],para=[],items=[],listTag=null,code=[],inCode=false;function flushPara(){if(para.length){out.push("<p>"+para.map(inlineMd).join("<br>")+"</p>");para=[];}}function flushList(){if(items.length){out.push("<"+listTag+">"+items.map(function(x){return "<li>"+x+"</li>";}).join("")+"</"+listTag+">");items=[];listTag=null;}}for(var i=0;i<lines.length;i++){var line=lines[i],m;if(/^```/.test(line)){if(inCode){out.push("<pre><code>"+esc(code.join("\n"))+"</code></pre>");code=[];inCode=false;}else{flushPara();flushList();inCode=true;}continue;}if(inCode){code.push(line);continue;}if(!line.trim()){flushPara();flushList();continue;}if(tableLike(line)&&i+1<lines.length&&(tableSep(lines[i+1])||tableLike(lines[i+1]))){var rows=[line],hasSep=false;i++;if(tableSep(lines[i])){rows.push(lines[i]);hasSep=true;i++;}while(i<lines.length&&lines[i].trim()&&tableLike(lines[i])){rows.push(lines[i]);i++;}i--;flushPara();flushList();out.push(renderTable(rows,hasSep));continue;}m=line.match(/^(#{1,6})\s+(.+)$/);if(m){flushPara();flushList();out.push("<h"+m[1].length+">"+inlineMd(m[2])+"</h"+m[1].length+">");continue;}if(/^\s*([-*_])\s*(\1\s*){2,}$/.test(line)){flushPara();flushList();out.push("<hr>");continue;}m=line.match(/^\s*>\s?(.+)$/);if(m){flushPara();flushList();out.push("<blockquote>"+inlineMd(m[1])+"</blockquote>");continue;}m=line.match(/^\s*[-*]\s+(.+)$/);if(m){flushPara();if(listTag&&listTag!=="ul"){flushList();}listTag="ul";items.push(inlineMd(m[1]));continue;}m=line.match(/^\s*\d+[.)]\s+(.+)$/);if(m){flushPara();if(listTag&&listTag!=="ol"){flushList();}listTag="ol";items.push(inlineMd(m[1]));continue;}para.push(line);}if(inCode){out.push("<pre><code>"+esc(code.join("\n"))+"</code></pre>");}flushPara();flushList();return out.join("");}~');
    ...
    append_line(l_html, 'function ask(){var q=prompt.value.trim(),m=model?model.value:"";if(!q){return;}add("user",esc(q));prompt.value="";var hold=add("agent","<p>Thinking...</p>");apex.server.process("CSIRO_CAAB_AGENT_ASK",{x01:q,x02:m},{dataType:"json"}).then(function(r){if(!r||!r.success){hold.querySelector(".afma-caab-markdown").innerHTML="<p>"+esc((r&&r.message)||"No response returned.")+"</p>";return;}var html=r.answerMarkdown?safeMd(r.answerMarkdown):(r.answerHtml||"");if(r.selectedServiceName){html="<p><strong>Model:</strong> "+esc(r.selectedServiceName)+"</p>"+html;}if(r.supportingHtml){html+="<details class=\"afma-caab-section\"><summary>Grounding details</summary>"+r.supportingHtml+"</details>";}hold.querySelector(".afma-caab-markdown").innerHTML=html;}).catch(function(err){hold.querySelector(".afma-caab-markdown").innerHTML="<p>"+esc(err&&err.message?err.message:err)+"</p>";});}');
